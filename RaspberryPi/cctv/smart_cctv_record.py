    ...
    while True:                                     # 무한루프
        ...
        if not ref:                     # 영상이 잘 받아지지 않았으면(ref가 거짓)
            ...
            # if start_record == True and is_record == False:  # 현재 녹화상태가 아니며 start_record가 참이면(녹화버튼 누름)
                ...
                # create a frame buffer to store the frames for the previous 10 seconds
                frame_buffer = []

                # loop over the previous 10 seconds of frames and add them to the frame buffer
                for i in range(150):
                    _, frame = capture.read()
                    frame_buffer.append(frame)

                # write the frames from the frame buffer to the video file
            ...
            # 녹화 버퍼가 10초 이상 차면 버퍼에 저장된 프레임을 비디오에 쓰기
            ...
            # 비디오 쓰기 중 에러 발생 시 종료
            if video is not None and video.isOpened() == False:
                break
